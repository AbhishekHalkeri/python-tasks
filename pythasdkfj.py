def insert():
    ...
    cursor=conn.cursor()
    cursor.execute('''insert into class8(Name,SID,marks) values ('abhay',04,89);''')
    print("record has been inserted.")
    conn.commit()
    conn.close()
def dyn_insert():
    conn=pc.connect(dbname='school',user='postgres',password='Abhi@12092003shek',host='localhost',port=5432)
    cursor=conn.cursor()
